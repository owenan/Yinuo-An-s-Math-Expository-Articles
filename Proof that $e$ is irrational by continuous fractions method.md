...
$$e = [2,1,2,1,1,4,1,1,6,1,1,8,\dots] \quad (1)$$

In fact, if we slightly transform the continued fraction on the right-hand side based on $1+\frac1{0+\frac1{1+\dots}}=2+\dots$, we obtain $e=[1,0,1,1,2,1,1,4,1,1,8,1,1,10,1,1,\dots]$. This expression exhibits a much clearer pattern, making it easier to define rigorously.

---

## Properties of Continued Fractions

A fundamental property of continued fractions is that its $n$-th convergent $[a_0,a_1,\dots,a_n]$ equals $\frac{p_n}{q_n}$, where $p_n$ and $q_n$ are recursively defined as:
...
$$[a_0,a_1]=a_0+\frac1{a_1} \quad (1.1)$$

$$[a_0,a_1,\dots,a_{n-1},a_n]=\left[a_0,a_1,\dots,a_{n-2},a_{n-1}+\frac1{a_n}\right] \quad (1.2)$$

$$[a_0,a_1,\dots,a_n]=a_0+\frac1{[a_1,\dots,a_n]}=[a_0,[a_1,\dots,a_n]] \quad (1.3)$$

Applying these properties repeatedly:

$$[a_0,a_1,\dots,a_n]=[a_0,[a_1,\dots,a_n]]=[a_0,[a_1,[a_2,\dots,a_n]]]$$

$$=[a_0,[a_1+\frac1{[a_2,\dots,a_n]}]]=[a_0,a_1+\frac1{[a_2,\dots,a_n]}]=[a_0,a_1,[a_2,\dots,a_n]]$$
...
$$[a_0,a_1,\dots,a_{m-1},a_m,a_{m+1}]=[a_0,a_1,\dots,a_{m-1},a_m+\frac1{a_{m+1}}]$$
...
$$[1,0,1,1,2,1,1,4,1,1,8,1,1,10,1,1,\dots]=[a_0,a_1,a_2,\dots]$$
...
$$\begin{aligned}
A_{n} &= \int_0^1\frac{x^n(x-1)^n}{n!}e^xdx, \\
B_{n} &= \int_0^1\frac{x^{n+1}(x-1)^n}{n!}e^xdx, \\
C_{n} &= \int_0^1\frac{x^n(x-1)^{n+1}}{n!}e^xdx.
\end{aligned}$$

> **Lemma 2.** For $n\ge 0$, $A_n=q_{3n}e-p_{3n}$, $B_n=p_{3n+1}-q_{3n+1}e$, and $C_n=p_{3n+2}-q_{3n+2}e$.

**Proof.** Based on our previously defined recurrence relations:
...
$$\begin{aligned}
A_{n} &= -B_{n-1}-C_{n-1} \quad & (2) \\
B_{n} &= -2nA_n+C_{n-1} \quad & (3) \\
C_{n} &= B_n-A_n \quad & (4)
\end{aligned}$$
...
$$e=\lim_{i\to\infty}\frac{p_i}{q_i}=[1,0,1,1,2,1,1,4,1,\dots]$$

This completes the proof. $\square$

---

## Convergence to Irrational Numbers

Now, we begin to prove that all infinite simple continued fractions converge to irrational numbers.

> **Lemma 3.** $C_k-C_{k-1}=\frac{(-1)^{k+1}}{q_kq_{k-1}}$, where $C_k = \frac{p_k}{q_k}$ represents the convergents defined in *Lemma 1*.

**Proof.** We prove this by induction. Let the proposition $P(n)$ be $p_nq_{n-1}-p_{n-1}q_n=(-1)^{n+1}$.
...
**Proof.** We prove this by induction. According to the definition of a simple continued fraction, all terms $a_k$ (except possibly $a_0$) are positive integers greater than or equal to $1$. Therefore:
...
**Proof.** According to Lemma 4, the denominators of the convergents are monotonically increasing, except for the possible case where $q_0=q_1=1$.
...
**Proof.** According to Lemma 5, we only need to show that $F_{k+1}>k$.

Assume that for $k\ge 5$, we have $F_{k+1}> k$ and $F_{k}> k-1$. Then:
...
**Proof.** Let $\epsilon>0$. For $m>n\ge\max\{5,\frac1\epsilon\}$, we have:

$$|C_m-C_n|\le|C_m-C_{m-1}|+\dots+|C_{n+1}-C_n|$$

$$=\frac1{q_mq_{m-1}}+\dots+\frac1{q_{n+1}q_n} \quad \text{(by Lemma 3)}$$

$$<\frac1{m(m-1)}+\dots+\frac1{(n+1)n} \quad \text{(by Lemma 6)}$$
...
**Proof.** Since $q_0=1>0$, and the denominators of a simple continued fraction are monotonically increasing, it naturally follows that $q_k>0$ for all $k\ge 0$. $\square$

> **Lemma 10.** The odd-indexed convergents of a simple continued fraction are monotonically decreasing.

**Proof.** Let $k\ge 3$ be an odd number. According to Lemma 8:
...
**Proof.** Let $k\ge 2$ be an even number. Then:
...
**Proof.** By Lemma 9, $q_kq_{k-1}>0$. By Lemma 3, $C_k-C_{k-1}=\frac{(-1)^{k+1}}{q_kq_{k-1}}$.
...
**Proof.** We will show that either $x\in [C_n,C_{n+1}]$ or $x\in [C_{n+1},C_n]$.
